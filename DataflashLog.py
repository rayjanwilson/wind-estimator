    ...
    def findLoiterChunks(logdata, minLengthSeconds=0, noRCInputs=True):
        ...
        for i in range(len(od.keys())):
            if od.values()[i][0] == "Loiter":
                ...
                chunkTimeSeconds = (int(DataflashLogHelper.getTimeAtLine(logdata,endLine))-int(DataflashLogHelper.getTimeAtLine(logdata,startLine))+1) / 1000.0
...
#                raise ValueError("Invalid log format for stdin: {}".format(format))
        ...
        if "GPS" in self.channels:
            ...
            if timeLabel == 'TimeUS':
                firstTimeGPS = int(firstTimeGPS) / 1000
                lastTimeGPS = int(lastTimeGPS) / 1000
